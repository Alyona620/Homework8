	...
	function getTemplate()
	{
		...
		if ($layout == 'default')
		{
			...
			foreach ($submissions as $SubmissionId => $submission)
			{
				...
				$replace = array_merge($replace, array('{global:date_added}', '{global:submissionid}', '{global:submission_id}', '{global:counter}', '{details}', '{detailspdf}','{global:confirmed}'));
				$with 	 = array_merge($with, array($submission['DateSubmitted'], $SubmissionId, $SubmissionId, $pagination->getRowOffset($i), '<a href="'.JRoute::_('index.php?option=com_rsform&view=submissions&layout=view&cid='.$SubmissionId.$Itemid).'">', '<a href="'.$pdf_link.'">',$submission['confirmed']));
