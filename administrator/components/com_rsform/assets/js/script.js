...
function toggleOrderSpans()
{
	...
	jQuery('table#componentPreview td.order').each(function(index, el) {
		var spans = jQuery(el).children();
		spans.removeAttr('style');
		if (index == 0)
			jQuery(spans[0]).css('visibility', 'hidden');
		else if (index == table.length - 1)
			jQuery(spans[1]).css('visibility', 'hidden');
	});
}
...
function toggleOrderSpansMp()
{
	...
	jQuery('table#mappingTable td.order').each(function(index, el) {
		var spans = jQuery(el).children();
		spans.removeAttr('style');
		if (index == 0)
			jQuery(spans[0]).css('visibility', 'hidden');
...
function toggleDropdown(what,extra)
{
	...
	//var parent		= jQuery('#' + nameid).parent();
	var quickfields = returnQuickFields();
	var parent = jQuery(what).parent();
	if (jQuery('#dropdown' + nameid).length == 0)
	{
	...
	var dropdown = jQuery('#dropdown' + nameid);
